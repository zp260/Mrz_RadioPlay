...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		label.textColor = [UIColor brownColor];//TEXT_COLOR;
...
//		label.shadowOffset = CGSizeMake(0.0f, 1.0f);
		...
		layer.frame = CGRectMake(30, RefreshViewHight - RefreshViewHight, 30.0f, 55.0f);
		...
		view.frame = CGRectMake(60, RefreshViewHight - 38.0f, 20.0f, 20.0f);
...
- (void)refreshLastUpdatedDate {
	...
	if ([_delegate respondsToSelector:@selector(egoRefreshTableHeaderDataSourceLastUpdated:)]) {
		...
		NSArray *array = [_delegate egoRefreshTableHeaderDataSourceLastUpdated:self];
		if ([[array objectAtIndex:1] intValue]>=[[array objectAtIndex:0] intValue]) {
            //已全部加载完成
            _lastUpdatedLabel.text=@"Have all been finished loading";
        }
		else
		_lastUpdatedLabel.text = [NSString stringWithFormat:@"Loaded:(%d)/Total(%d)",[[array objectAtIndex:1] intValue],[[array objectAtIndex:0] intValue]];
...
- (void)setState:(EGOPullRefreshState)aState{
	...
	switch (aState) {
		case EGOOPullRefreshPulling:
			//松开即可加载
			_statusLabel.text = NSLocalizedString(@"Release can load...", @"Release can load...");
			[CATransaction begin];
		...
		case EGOOPullRefreshNormal:
			...
			//上拉即可加载
			_statusLabel.text = NSLocalizedString(@"Pull up to load...", @"Pull up to load...");
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
        ...
        [self performSelector:@selector(reloadMoreData) withObject:nil afterDelay:0.0001];
